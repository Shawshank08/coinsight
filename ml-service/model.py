...
from features import MIN_SUPERVISED_ROWS, feature_columns, latest_feature_row, training_frame
...
class BitcoinPriceModel:
    ...
    def fit(self, daily_ohlcv: pd.DataFrame) -> dict[str, float]:
        ...
        self._residual_std = float(np.std(residuals, ddof=1)) if len(residuals) > 1 else float(np.std(residuals))
